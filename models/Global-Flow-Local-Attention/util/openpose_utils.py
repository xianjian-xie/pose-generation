# openpose_utils
import numpy as np
from skimage.draw import line_aa, polygon, circle_perimeter_aa
...
def draw_joint(colors, pose_joints, joint_line_list, radius=2):
    ...
    for i in range(pose_joints.shape[1]):
        if pose_joints[0,i] == MISSING_VALUE or pose_joints[1,i] == MISSING_VALUE:
            continue
        yy, xx = morphology.disk((pose_joints[0,i], pose_joints[1,i]), radius=radius, shape=im_size)
...
def openpose25_to_coco17(pose_joints_25):
    pose_joints_17 = np.zeros((2,17)).astype(pose_joints_25.dtype)
    i = 0
    for key in COCO_17:
